"""For example, 2 is written as II in Roman numeral, just two ones added together.
 12 is written as XII, which is simply X + II.
  The number 27 is written as XXVII,
   which is XX + V + II.

Roman numerals are usually written
largest to smallest from left to right.
However, the numeral for four is not IIII.
Instead, the number four is written as
IV. Because the one is before the five
we subtract it making four. The same
principle applies to the number nine,
which is written as IX. There are six
instances where subtraction is used:
"""


def roman_number(row):
    roman_numeral = {
        'I': 1,
        'V': 5,
        'X': 10,
        'L': 50,
        'C': 100,
        'D': 500,
        'M': 1000
        }
    number = 0
    last_number = 0
    for i in reversed(row):
        value = roman_numeral[i]
        if value < last_number:
            number -= value
        else:
            number += value
        last_number = value
    return number


data = input("Rim raqamlarini kiriting: ")
print(f"{data} = {roman_number(data.upper())}")

